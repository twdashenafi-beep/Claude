// Moving a task between Day, Week and Month.
//
// A task's horizon was fixed when it was made and the columns filter on it, so
// something that turned out to be a this-week job rather than a today job had
// nowhere to go.
//
...
// ── Following it ──
await A.page.getByLabel('Switch to Week').click();
await A.page.waitForTimeout(900);
ok('the offered link goes to the scope it moved to', await shows(),
   (await body(A.page)).slice(0, 200));

// ── And it stays there ──
await A.page.getByLabel('Show day tasks').click();
await A.page.waitForTimeout(500);
ok('it is no longer under Day', !(await shows()));
await A.page.getByLabel('Show month tasks').click();
await A.page.waitForTimeout(500);
ok('and it did not land under Month either', !(await shows()));
...
await A.page.getByLabel('Show under Month').click();
const save = A.page.locator('text=Save').first();
if (await save.count()) await save.click();
await A.page.waitForTimeout(1200);

ok('a scope changed in the task sheet takes effect', !(await shows()),
   (await body(A.page)).slice(0, 200));
await A.page.getByLabel('Show month tasks').click();
await A.page.waitForTimeout(600);
ok('and it is where it was moved to', await shows());
