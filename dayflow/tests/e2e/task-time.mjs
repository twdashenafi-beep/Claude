// Setting a task's time, when creating it and afterwards.
//
// The task sheet printed the date and time and offered no way to alter them,
// so a time could be chosen once, at creation, and never corrected. Both sheets
// now use the same controls, which is also the only way they stay in step.
//
...
const to24 = label => {
  const [, hh, mm, period] = label.match(/^(\d{1,2}):(\d{2}) (AM|PM)$/);
  let h = Number(hh) % 12;
  if (period === 'PM') h += 12;
  return `${String(h).padStart(2, '0')}:${mm}`;
};
ok('the picker shows the time being chosen', !!chosen, String(chosen));

await A.page.getByLabel('Use this time').click();
await A.page.waitForTimeout(500);

let text = await body(A.page);
ok('the chip shows the chosen time, not the word Time',
   text.includes(chosen), `wanted ${chosen}`);
...
// The section label is styled uppercase, and innerText reflects that.
ok('the task sheet opens', /date & time/i.test(text), text.slice(0, 200));

const timeChip = A.page.getByLabel(new RegExp(`Due at ${chosen.replace(/[.*+?^$()|[\]\\]/g, '\\$&')}`));
ok('the existing time is offered as a control, not just printed',
   (await timeChip.count()) === 1);
...
// Save the task sheet.
const save = A.page.locator('text=Save').first();
